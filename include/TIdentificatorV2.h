...
class TIdentificatorV2 {
 public:
  ...
  Double_t Xf(Int_t k, Bool_t kind = 0) {
    ...
    PlCM = (TMath::Sqrt(Pl2(k, kind)) - Zh(k, kind) * Nu(kind) * TMath::Sqrt(Q2(kind) + Nu(kind) * Nu(kind)) / (Nu(kind) + kMassProton)) *
           (Nu(kind) + kMassProton) / W(kind);
    PlCMmax = TMath::Sqrt(TMath::Power(W(kind), 4) + TMath::Power(kMassPiPlus * kMassPiPlus - kMassNeutron * kMassNeutron, 2) -
                          2 * W(kind) * W(kind) * (kMassPiPlus * kMassPiPlus + kMassNeutron * kMassNeutron)) /
              (2 * W(kind));
    return PlCM / PlCMmax;
  }
  ...
  Bool_t SampFracCheck(Int_t k, TString dataKind, TString targetOption, Double_t NSigma = 2.5) {
    ...
    // final step
    if (TMath::Abs(E / p - MeanE) < NSigma * SigmaE) {
      return true;
    }  // closure
    return false;
  }
